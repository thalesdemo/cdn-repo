import {
  populateInputField,
  observeDOMChanges,
  insertElementBelowAnchor,
  appendChildToElement,
} from "../utils/tulip-customizer-commons.js";
...
export function setupBadgeSystem(config) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const tasks = [
      ...
      {
        check: () =>
          document.getElementById(config.video.elementId) &&
          !qrScannerInitialized,
        action: () => {
...
function insertVideoContainer(config) {
  ...
  const videoContainer = insertElementBelowAnchor(
    anchorSelector,
    config.video.containerId,
    "div",
  ...
  const videoElement = appendChildToElement(
    config.video.containerId,
...
function setupQrScannerAndPopulateInput(qrConfig) {
  ...
  const qrScanner = initializeQrScanner(qrConfig)
    .then(() => {
